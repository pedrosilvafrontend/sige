...
import { ManagersFormDialogComponent } from './dialogs/form-dialog/managers-form-dialog.component';
import { ManagersDeleteDialogComponent } from './dialogs/delete/managers-delete-dialog.component';
import {
  MAT_DATE_LOCALE,
  MatOptionModule,
  MatRippleModule,
...
export class ManagersComponent implements OnInit, OnDestroy {
  ...
  columnsLabels = ['name', 'email', 'gender', 'mobile', 'school', 'actions'].map((key: string) => this.translate.instant(key));

  columnDefinitions = (() => {
    const [name, email, gender, mobile, school, actions] = this.columnsLabels;
    return [
  ...
  openDialog(action: 'add' | 'edit', data?: User) {
    const dialogRef = this.dialog.open(ManagersFormDialogComponent, {
      width: '60vw',
      maxWidth: '100vw',
  ...
  deleteItem(row: User) {
    const dialogRef = this.dialog.open(ManagersDeleteDialogComponent, { data: row, disableClose: true });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  loadData() {
    ...
    this.service.getAll('managers').subscribe({
      next: (data) => {
  ...
  exportExcel() {
    ...
    const exportData = this.dataSource.filteredData.map((x) => ({
      ...
      [email]: x.email,
      [gender]: x.gender,
      [birthDate]: x.birthDate ? format(new Date(x.birthDate), 'yyyy-MM-dd') || '' : '',
      [mobile]: x.mobile,
